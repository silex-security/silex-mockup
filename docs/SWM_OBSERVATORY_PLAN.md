...
原页面把 L2 与 L3 画成相交（× 号，"互不实例化"），这是错的。已改为单一串联链
**L1 → L2 → L3 → L4**：L3 的 agentic 组件挂在它实际部署的 L2 域包之下（从 L4 运行时实例反推），
跨域组件归入新的 "Cross-domain & Horizontal" 包。层级关系不再靠边的顺序推断，而是每个节点显式带
`parent`，构建脚本对"父节点必须在同层或上一层"做硬校验，违规即非零退出。Ontology Layers tab 随之
重做为四段层带 + 真实流带，并与 Explorer 共享同一个层级状态。
